    ...
    describe "OPCODE" {
        describe "LD" {
            describe "ld" {
                ...
                fn test(arg: Args) {
                    ...
                    if &arg.r2 == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC, want);
                    } else if &arg.r2 == "dd" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC, want);
                        cpu.bus.lock().unwrap().write(cpu.reg.PC + 1, want + 1);
                    } else {
            ...
            describe "ld,(aa),sp" {
                ...
                fn test(arg: Args) {
                    ...
                    cpu.bus.lock().unwrap().write(cpu.reg.PC, 0x34);
                    cpu.bus.lock().unwrap().write(cpu.reg.PC + 1, 0x12);
                    let handler = &opcode.handler;
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    assert_eq!(cpu.bus.lock().unwrap().read(0x1234), 0x67);
                    assert_eq!(cpu.bus.lock().unwrap().read(0x1235), 0x45);
                }
            }
            ...
            describe "ld,hl,sp+8" {
                ...
                fn test(arg: Args) {
                    ...
                    cpu.bus.lock().unwrap().write(cpu.reg.PC, 0xE1);
                    let handler = &opcode.handler;
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    ...
                    cpu.bus.lock().unwrap().write(cpu.reg.PC, 0x0F);
                    let handler = &opcode.handler;
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
        ...
        describe "JR" {
            describe "jr" {
                ...
                fn test(arg: Args) {
                    ...
                    // positive
                    cpu.reg.PC = 0x100;
                    cpu.bus.lock().unwrap().write(cpu.reg.PC,  0x10);
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    assert_eq!(cpu.load(&"PC".to_string()), 0x0111);

                    // nagative
                    cpu.reg.PC = 0x100;
                    cpu.bus.lock().unwrap().write(cpu.reg.PC,  0xFE); // -2
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
        ...
        describe "ADD" {
            describe "add" {
                ...
                fn test(arg: Args) {
                    ...
                    if opcode.r2.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC, 0x0E);
                    } else {
                    ...
                    cpu.store(&opcode.r1, 0xE1);
                    if opcode.r2.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC, 0x1F);
                    } else {
            ...
            describe "addr16d" {
                ...
                fn test(arg: Args) {
                    ...
                    cpu.bus.lock().unwrap().write(cpu.reg.PC, 0x0D);
                    cpu.reg.F.unpack(0b00000000);
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    ...
                    // negative half carry
                    cpu.store(&opcode.r1, 0x0FD1);
                    cpu.bus.lock().unwrap().write(cpu.reg.PC, 0x8F);
                    cpu.reg.F.unpack(0b00000000);
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    ...
                    cpu.bus.lock().unwrap().write(cpu.reg.PC, 0x80);
                    cpu.reg.F.unpack(0b00000000);
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
        ...
        describe "SUB" {
            describe "sub" {
                ...
                #[rstest(arg,
                    case(Args{opcode: 0x90, r1: "A".to_string(), r2:  "B".to_string()}),
                    case(Args{opcode: 0x91, r1: "A".to_string(), r2:  "C".to_string()}),
                    case(Args{opcode: 0x92, r1: "A".to_string(), r2:  "D".to_string()}),
                    case(Args{opcode: 0x93, r1: "A".to_string(), r2:  "E".to_string()}),
                    case(Args{opcode: 0x94, r1: "A".to_string(), r2:  "H".to_string()}),
                    case(Args{opcode: 0x95, r1: "A".to_string(), r2:  "L".to_string()}),
                    case(Args{opcode: 0x96, r1: "A".to_string(), r2: "(HL)".to_string()}),
                    case(Args{opcode: 0x97, r1: "A".to_string(), r2:  "A".to_string()}),
                    case(Args{opcode: 0xD6, r1: "A".to_string(), r2:  "d".to_string()}),
                )]
                fn test(arg: Args) {
                    let mut cpu = common::fixture::setup_cpu();
                    ...
                    cpu.store(&opcode.r1, 0xE1);
                    if opcode.r2.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC, 0x01);
                    } else {
                        cpu.store(&opcode.r2, 0x01);
                    }
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    assert_eq!(cpu.load(&opcode.r1), 0xE0);
                    assert_eq!(cpu.reg.F.pack(), 0b01000000);

                    // half carry
                    cpu.store(&opcode.r1, 0xE1);
                    if opcode.r2.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC, 0x02);
                    } else {
                        cpu.store(&opcode.r2, 0x02);
                    }
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    assert_eq!(cpu.load(&opcode.r1), 0xDF);
                    ...
                    cpu.store(&opcode.r1, 0xE1);
                    if opcode.r2.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC, 0xE2);
                    } else {
                        cpu.store(&opcode.r2, 0xE2);
                    }
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    assert_eq!(cpu.load(&opcode.r1), 0xFF);
                    assert_eq!(cpu.reg.F.pack(), 0b01110000);
                }
            }
        }
        ...
        describe "SBC" {
            describe "sbc" {
                ...
                #[rstest(arg,
                    case(Args{opcode: 0x98, r1: "A".to_string(), r2:  "B".to_string()}),
                    case(Args{opcode: 0x99, r1: "A".to_string(), r2:  "C".to_string()}),
                    case(Args{opcode: 0x9A, r1: "A".to_string(), r2:  "D".to_string()}),
                    case(Args{opcode: 0x9B, r1: "A".to_string(), r2:  "E".to_string()}),
                    case(Args{opcode: 0x9C, r1: "A".to_string(), r2:  "H".to_string()}),
                    case(Args{opcode: 0x9D, r1: "A".to_string(), r2:  "L".to_string()}),
                    case(Args{opcode: 0x9E, r1: "A".to_string(), r2: "(HL)".to_string()}),
                    case(Args{opcode: 0x9F, r1: "A".to_string(), r2:  "A".to_string()}),
                    case(Args{opcode: 0xDE, r1: "A".to_string(), r2:  "d".to_string()}),
                )]
                fn test(arg: Args) {
                    let mut cpu = common::fixture::setup_cpu();
                    ...
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    assert_eq!(cpu.load(&opcode.r1), 0xE1);
                    assert_eq!(cpu.reg.F.pack(), 0b01000000);
                    ...
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    assert_eq!(cpu.load(&opcode.r1), 0xD1);
                    assert_eq!(cpu.reg.F.pack(), 0b01100000);
                    ...
                    handler(&mut cpu, opcode.r1.to_string(), opcode.r2.to_string());
                    assert_eq!(cpu.load(&opcode.r1), 0xD2);
                    assert_eq!(cpu.reg.F.pack(), 0b01100000);
                    ...
                    cpu.store(&opcode.r1, 0xF1);
                    if opcode.r2.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC, 0xF0);
                    } else {
            ...
            describe "and" {
                ...
                fn test(arg: Args) {
                    ...
                    if opcode.r1.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC,  0b11110000);
                    } else {
                    ...
                    cpu.reg.A = 0b11110000;
                    if opcode.r1.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC,  0b00001111);
                    } else {
                    ...
                    cpu.reg.A = 0b11110000;
                    if opcode.r1.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC,  0b10100000);
                    } else {
        ...
        describe "OR" {
            describe "or" {
                ...
                fn test(arg: Args) {
                    ...
                    cpu.reg.A = 0b11110000;
                    if opcode.r1.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC,  0b00000101);
                    } else {
                    ...
                    if opcode.r1.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC,  0b00000000);
                    } else {
        ...
        describe "XOR" {
            describe "xor" {
                ...
                fn test(arg: Args) {
                    ...
                    cpu.reg.A = 0b11110000;
                    if opcode.r1.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC,  0b01010101);
                    } else {
        ...
        describe "CP" {
            describe "cp" {
                ...
                fn test(arg: Args) {
                    ...
                    cpu.reg.A = 0x12;
                    if opcode.r1.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC,  0x12);
                    } else {
                    ...
                    cpu.reg.A = 0x12;
                    if opcode.r1.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC,  0x13);
                    } else {
                    ...
                    cpu.reg.A = 0x12;
                    if opcode.r1.as_str() == "d" {
                        cpu.bus.lock().unwrap().write(cpu.reg.PC,  0x03);
                    } else {
        ...
        describe "CALL" {
            describe "call" {
                ...
                fn test(arg: Args) {
                    ...
                    assert_eq!(cpu.bus.lock().unwrap().read(0xFFFA), 0x7A);
                    assert_eq!(cpu.bus.lock().unwrap().read(0xFFFB), 0x56);
                }
            }
            ...
            describe "push" {
                ...
                fn test(arg: Args) {
                    ...
                    if opcode.r1.as_str() == "AF" {
                        assert_eq!(cpu.bus.lock().unwrap().read(cpu.reg.SP), 0x30);
                    } else {
                        assert_eq!(cpu.bus.lock().unwrap().read(cpu.reg.SP), 0x34);
                    }
                    assert_eq!(cpu.bus.lock().unwrap().read(cpu.reg.SP + 1), 0x12);
                }
            }
        }
        ...
        describe "RET" {
            describe "ret" {
                ...
                fn test(arg: Args) {
                    ...
                    cpu.reg.SP = 0xFFFC;
                    cpu.reg.PC = 0x5678;
                    cpu.bus.lock().unwrap().write(cpu.reg.SP, 0x34);
                    cpu.bus.lock().unwrap().write(cpu.reg.SP + 1, 0x12);
